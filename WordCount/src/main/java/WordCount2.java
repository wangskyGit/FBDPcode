...
public class WordCount2 {
    ...
    public static class SortMapper
            extends Mapper<Object,Text,Pair, NullWritable>{
        private final Pair p_key = new Pair();
        ...
        public void map(Object key, Text value, Context context) throws IOException, InterruptedException
        {
            ...
            if (tokenizer.hasMoreTokens())
            {
                ...
                p_key.set(new Text(left), right);
                context.write(p_key, NullWritable.get());
            }
        }
    }
    ...
    public static class SortReducer extends Reducer<Pair, NullWritable, IntWritable, Text>
    {
        private final Text left = new Text();
        private static final Text SEPARATOR = new Text("------------------------------------------------");

        public void reduce(Pair key, NullWritable values,Context context) throws IOException, InterruptedException
        {
            left.set(key.getA().toString());
            //将排名前100的单词输出
            for (int i=0;i<100;i++)
            {
                context.write(new IntWritable(i+1), left);
            }
        }
    }
